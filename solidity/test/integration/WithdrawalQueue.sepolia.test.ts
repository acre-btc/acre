...
async function sepoliaIntegrationFixture() {
    ...
    const [deployer, thirdParty, treasury] = await helpers.signers.getUnnamedSigners()
    ...
    const midasVault = await ethers.getContractAt("IVault", SEPOLIA_MIDAS_VAULT_ADDRESS)
    ...
    await midasAllocator.connect(deployer).setWithdrawalQueue(await withdrawalQueue.getAddress())
    ...
    describe("Accounting and Withdrawal Flow Tests", () => {
        ...
        describe("Simple Redemption Flow (requestRedeem)", () => {
            ...
            it("should handle simple redeem request without creating withdrawal request record", async () => {
                ...
                const finalTreasuryShares = await vaultSharesToken.balanceOf(treasury.address)
                ...
                expect(actualFeeTransferred).to.be.approximately(expectedExitFee, to1e6(0.001))
            })
        ...
        describe("Bridge Request Tracking (requestRedeemAndBridge)", () => {
            it("should handle complete withdrawal - event amount should match original deposit", async () => {
                ...
                if (event) {
                    ...
                    expect(eventTbtcAmount).to.be.approximately(depositAmount, to1e6(0.000001))
                    expect(eventTbtcAmount).to.be.approximately(expectedTbtcAmount, to1e6(0.000001))
                }
            ...
            it("should create and track withdrawal request with proper data", async () => {
                ...
                const tx = await withdrawalQueue.connect(depositor2).requestRedeemAndBridge(
                    sharesToRedeem,
                    walletPubKeyHash
                )
            ...
            it("should handle multiple withdrawal requests correctly", async () => {
                ...
                await withdrawalQueue.connect(depositor).requestRedeemAndBridge(
                    sharesToRedeem1,
                    walletPubKeyHash1
                )

                // Create second request
                await withdrawalQueue.connect(depositor2).requestRedeemAndBridge(
                    sharesToRedeem2,
                    walletPubKeyHash2
                )
                ...
                const request2 = await withdrawalQueue.withdrawalRequests(initialCount + BigInt(1))
        ...
        describe("Fee Structure and Accounting", () => {
            it("should correctly calculate and transfer exit fees for simple redeem", async () => {
                ...
                expect(actualFeeTransferred).to.be.approximately(expectedFee, to1e6(0.001))
            })
            ...
            it.skip("should handle zero exit fees when fee is set to 0", async () => {
                ...
                await withdrawalQueue.connect(depositor2).requestRedeem(sharesToRedeem, depositor2.address)
        ...
        describe("MidasAllocator Integration", () => {
            it("should properly interact with MidasAllocator.withdraw()", async () => {
                ...
                const initialAllocatorShares = await vaultSharesToken.balanceOf(await midasAllocator.getAddress())

                await withdrawalQueue.connect(depositor).requestRedeem(sharesToRedeem, depositor.address)

                const finalAllocatorShares = await vaultSharesToken.balanceOf(await midasAllocator.getAddress())
        ...
        describe("State and Data Integrity", () => {
            it("should maintain consistent state across multiple mixed requests", async () => {
                ...
                await withdrawalQueue.connect(depositor2).requestRedeemAndBridge(bridgeShares, walletPubKeyHash)
